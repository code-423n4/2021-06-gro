...
contract MockCurveDeposit is ICurve3Deposit {
    ...
    function add_liquidity(
        ...
        for (uint256 i; i < N_COINS; i++) {
            ...
            amount =
                ((uamounts[i] * (10**(18 - decimals[i]))) * vpSingle[i]) /
                (10**18);
        }
    ...
    {
        require(
            PoolToken.balanceOf(msg.sender) > amount,
    ...
    function remove_liquidity_imbalance(
        ...
        require(
            PoolToken.balanceOf(msg.sender) > max_burn_amount,
            "remove_liquidity: !balance"
        );
    ...
    function remove_liquidity_one_coin(
        ...
        min_amount;
        require(
            PoolToken.balanceOf(msg.sender) > _token_amount,
            "remove_liquidity: !balance"
        );
        uint256 outAmount =
            ((_token_amount * (10**18)) / vpSingle[uint256(i)]) /
                PRECISION_MUL[uint256(i)];
